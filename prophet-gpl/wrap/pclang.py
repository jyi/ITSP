...
def preprocessGen(src_file, out_file, args, idx):
    ...
    print "[preprocessGen] Invoking: " + cmd;
...
def rewriteSourceGen(src_file, out_file, args, idx):
    ...
    print "[rewriteSourceGen] Invoking: " + cmd;
    ret = subprocess.call(cmd, shell=True);
    return ret;
...
def rewriteSource(src_file, out_file, text_file, args, idx):
    ...
    print "[rewriteSource] Invoking: " + cmd;
    ret = subprocess.call(cmd, shell=True);
    return ret;
...
def finalCompile(src_file, args, idx):
    ...
    ret = subprocess.call(cmd, shell = True);
    print "[finalCompile] invoking: " + cmd;
    return ret;
...
def genTmpFilename():
    cond = True;
    while cond:
        ret = os.environ['PROPHET_TMP'] + "/pclang_";
        for i in range(0, 8):
