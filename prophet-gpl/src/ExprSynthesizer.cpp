      ...
      if (partial) {
        for (TestCaseSetTy::iterator it = negative_cases.begin();
          ...
          if (testOneCase(env, *it)) {
            outlog_printf(2, "Passed Negative Case %lu\n", *it);
            std::ofstream file;
            file.open("fixed-test-case");
            file << *it << "\n";
            file.close();
            return true;
          }
...
public:
    BasicTester(BenchProgram &P, bool learning, SourceContextManager &M, bool naive, bool partial):
...
public:
    StringConstTester(BenchProgram &P, bool learning, SourceContextManager &M,
                      bool naive, bool partial):
      BasicTester(P, learning, M, naive, partial), candidate_strs(), infos() { }
    ...
    virtual bool test(const BenchProgram::EnvMapTy &env, unsigned long id) {
        ...
        for (TestCaseSetTy::iterator it = negative_cases.begin();
                it != negative_cases.end(); it++) {
            char *prophet_tmp_dir = getenv("PROPHET_TMP");
            std::string tmp_out = prophet_tmp_dir + std::string("/__out.str");
            std::string tmp_exp = prophet_tmp_dir + std::string("/__exp.str");
...
class ConditionSynthesisTester : public BasicTester {
    ...
    std::vector<unsigned long> parseBranchRecord() {
        ...
        res.clear();
        char *prophet_tmp_dir = getenv("PROPHET_TMP");
        std::string isneg_tmpfile = prophet_tmp_dir + std::string("/neg.tmp");
        FILE *f = fopen(isneg_tmpfile.c_str(), "r");
        //llvm::errs() << "New code\n" << new_code << "\n";
        if (!f) {
    ...
    void writeBranchRecordTerminator() {
        char *prophet_tmp_dir = getenv("PROPHET_TMP");
        std::string isneg_tmpfile = prophet_tmp_dir + std::string("/neg.tmp");
        FILE*f = fopen(isneg_tmpfile.c_str(), "a");
        fprintf(f, " 0");
        fclose(f);
    }
    ...
    void writeBranchRecord(const std::map<unsigned long, std::vector<unsigned long> > &negative_records, unsigned long idx) {
        char *prophet_tmp_dir = getenv("PROPHET_TMP");
        std::string isneg_tmpfile = prophet_tmp_dir + std::string("/neg.tmp");
        FILE *f = fopen(isneg_tmpfile.c_str(), "w");
    ...
    bool testNegativeCases(const BenchProgram::EnvMapTy &env,
        ...
        for (TestCaseSetTy::iterator case_it = negative_cases.begin();
            ...
            char *prophet_tmp_dir = getenv("PROPHET_TMP");
            testEnv.insert(std::make_pair("TMP_FILE", isneg_tmpfile.c_str()));
            int ret = system((std::string("rm -rf ") + isneg_tmpfile.c_str()).c_str());
            assert( ret == 0);
    ...
    void parseValueRecord(std::vector<std::vector<long long> > &vec, size_t expect_size) {
        vec.clear();
        char *prophet_tmp_dir = getenv("PROPHET_TMP");
        std::string isneg_recordfile = prophet_tmp_dir + std::string("/neg.log");
        FILE* f = fopen(isneg_recordfile.c_str(), "r");
    ...
    bool collectValues(const BenchProgram::EnvMapTy &env, const RepairCandidate &candidate,
        ...
        for (TestCaseSetTy::iterator tit = negative_cases.begin();
            ...
            testEnv.insert(std::make_pair("NEG_ARG", isneg_tmpfile.c_str()));
            testEnv.insert(std::make_pair("TMP_FILE", isneg_recordfile.c_str()));
...
public:
    ConditionSynthesisTester(BenchProgram &P, bool learning, SourceContextManager &M,
                             bool full_synthesis, bool partial):
      BasicTester(P, learning, M, false, partial),
...
public:
    TestBatcher(BenchProgram &P, bool naive,
                bool learning, bool partial, FeatureParameter *FP):
      P(P), naive(naive), learning(learning && !naive), partial(partial), FP(FP),
      res(), succCandidates(), candidateMap(), cur_size(0),
    total_cnt(0) { }
...
bool ExprSynthesizer::workUntil(size_t candidate_limit, size_t time_limit,
    ...
    TestBatcher TB(P, naive, learning, partial, FP);
    std::vector<BasicTester*> testers;
    testers.clear();
    testers.push_back(new ConditionSynthesisTester(P, learning, M, full_synthesis, partial));
    testers.push_back(new StringConstTester(P, learning, M, naive, partial));
    testers.push_back(new BasicTester(P, learning, M, naive, partial));
