...
class RepairCandidateGeneratorImpl : public RecursiveASTVisitor<RepairCandidateGeneratorImpl> {
    ...
    IfStmt* addGuard(Stmt *then, Stmt *elsev = nullptr) {
      BinaryOperator *BO =
        new(*ctxt) BinaryOperator(getNewIntegerLiteral(ctxt, 1),
                                  getNewIntegerLiteral(ctxt, 0), BO_NE, ctxt->IntTy,
                                  VK_RValue, OK_Ordinary, SourceLocation(), false);
      IfStmt *new_IF = new(*ctxt) IfStmt(*ctxt, SourceLocation(), NULL, BO, then,
                                         SourceLocation(), elsev);
      return new_IF;
    }
    ...
    bool ignore(Stmt *n) {
      ...
      unsigned line_num = loc.getLineNumber(M);
      for (std::vector<unsigned>::const_iterator i = lines.begin();
           i != lines.end(); i++) {
        if (*i == line_num) return false;
      }
      llvm::errs() << "ignore line num " << line_num << "\n";
      return true;
    }
    ...
    void genAddMemset(Stmt* n, bool is_first) {
        ...
        for (size_t i = 0; i < exprs.size(); i++) {
            ...
            if (L->isValidStmt(CE, NULL)) {
                RepairCandidate rc;
                rc.actions.clear();
                rc.actions.push_back(RepairAction(loc, RepairAction::InsertMutationKind,
                                                  addGuard(CE, n)));
                if (learning)
                    rc.score = getLocScore(n);
                else
    ...
    void genReplaceStmt(Stmt* n, bool is_first) {
        ...
        if (llvm::isa<Expr>(n)) {
            ...
            for (std::set<Stmt*>::iterator it = res.begin(); it != res.end(); ++it) {
                RepairCandidate rc;
                rc.actions.clear();
                rc.actions.push_back(RepairAction(loc, RepairAction::ReplaceMutationKind,
                                                  addGuard(*it, n)));
                // FIXME:
                if (learning)
                    rc.score = getLocScore(n);
        ...
        for (std::set<Expr*>::iterator it = exprs.begin(); it != exprs.end(); ++it) {
            ...
            for (std::set<Stmt*>::iterator it2 = stmts.begin(); it2 != stmts.end(); ++it2) {
                ...
                rc.actions.push_back(RepairAction(loc,
                                                  RepairAction::InsertMutationKind,
                                                  addGuard(*it2)));
                if (learning) {
                    rc.score = getLocScore(n);
                }
...
public:
    RepairCandidateGeneratorImpl(SourceContextManager &M, const std::string &file,
            ...
            bool naive, bool learning, bool not_fix_cf, bool not_fix_df):
        ctxt(M.getSourceContext(file)), M(M), file(file), loc_map(locs), lines(lines),
        hinfo(M.getInternalHandlerInfo(ctxt)), naive(naive), learning(learning),
        not_fix_cf(not_fix_cf), not_fix_df(not_fix_df) {
...
std::string RepairCandidate::toString(SourceContextManager &M) const {
    ...
    for (size_t i = 0; i < actions.size(); i++)
      if (!M.isNewStmt(actions[i].loc.stmt)) {
        RepairAction ra = actions[i];
        sout << "At location " << actions[i].loc.toString(M) << "\n";
        sout << "Repair action kind: " << ra.getKindAsString() << "\n";
      }
...
RepairCandidateGenerator::RepairCandidateGenerator(SourceContextManager &M, const std::string &file,
           ...
           bool not_fix_cf, bool not_fix_df) {
  impl = new RepairCandidateGeneratorImpl(M, file, locs, lines,
                                          naive, learning, not_fix_cf, not_fix_df);
}
