...
public:
    RepairSearchEngine(BenchProgram& P, ErrorLocalizer *L, bool naive, bool learning, 
                       bool partial, bool not_fix_cf, bool not_fix_df, FeatureParameter *FP,
                       std::vector<unsigned>& lines)
        : P(P), negative_cases(P.getNegativeCaseSet()), positive_cases(P.getPositiveCaseSet()), 
      naive(naive), learning(learning && !naive), partial(partial),
      not_fix_cf(not_fix_cf), not_fix_df(not_fix_df), 
      FP(FP), lines(lines), GeoP(0.08), random(false), summaryFile(""), timeout_limit(0) {
        this->L = L;
        bugged_files.clear();
