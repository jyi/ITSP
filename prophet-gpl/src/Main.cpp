...
llvm::cl::opt<bool> PartialRepair("partial",
        llvm::cl::desc("Accepts a partial repair that passes at least one negative test and all positive tests"));
...
llvm::cl::opt<bool> NotFixControlFlow("not-fix-control-flow",
        llvm::cl::desc("Repair only a data-flow error"));
llvm::cl::opt<bool> NotFixDataFlow("not-fix-data-flow",
        llvm::cl::desc("Repair only a control-flow error"));
llvm::cl::list<unsigned> Lines("lines",
                               llvm::cl::desc("suspicious lines"));
...
int main(int argc, char* argv[]) {
   ...
   if (InitOnly) {
    ...
    RepairSearchEngine E(*P, L, NaiveRepair.getValue(), learning, PartialRepair.getValue(),
                         NotFixControlFlow.getValue(), NotFixDataFlow.getValue(), FP, 
                         Lines);
