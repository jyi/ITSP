...
void ProfileErrorLocalizer::clearProfileResult() {
    ...
    std::string cmd = "rm -rf " + std::string(prophet_tmp_dir) + "/__run*.log";
...
std::map<SourcePositionTy, ProfileInfoTy> ProfileErrorLocalizer::parseProfileResult() {
    char *prophet_tmp_dir = getenv("PROPHET_TMP");
    if (LI == NULL) {
        std::string index_file = prophet_tmp_dir + std::string("/__index.loc");
        LI = new LocationIndex(index_file);
    }

    std::map<SourcePositionTy, ProfileInfoTy> M;
    M.clear();
    DIR* dp = opendir(prophet_tmp_dir);    
    struct dirent *dirp;
    while (((dirp = readdir(dp)))) {
        ...
        std::ifstream fin((std::string(prophet_tmp_dir) + "/" + nstr).c_str(), std::ifstream::in);
...
void clearTmpDirectory() {
    char *prophet_tmp_dir = getenv("PROPHET_TMP");
    std::string cmd = std::string("rm -rf ") + prophet_tmp_dir + "/__* " + 
      prophet_tmp_dir + "/pclang*";
...
ProfileErrorLocalizer::ProfileErrorLocalizer(BenchProgram &P,
    ...
    if (skip_build) {
        ...
        char *prophet_tmp_dir = getenv("PROPHET_TMP");
        std::string index_file = prophet_tmp_dir + std::string("/__index.loc");
        envMap["INDEX_FILE"] = index_file.c_str();
        clearTmpDirectory();
